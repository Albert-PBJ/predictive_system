...
All three map items → `CompetitorMarketData`, then run a **quality gate** (`scrapers/validation.py`) that **discards** non-plausible rows before `bulk_create`, and preserve the full Apify JSON in `raw_metadata`.
